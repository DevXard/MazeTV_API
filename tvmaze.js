  ...
  for (let show of shows) {
    ...
    let $item = $(
      `<div class="col-md-6 col-lg-3 Show" data-show-id="${show.show.id}">
         ...
         <img class="card-img-top" src=${checkForImg(show.show.image)}>
           <div class="card-body">
             <h5 class="card-title">${show.show.name}</h5>
             <p class="card-text">${show.show.summary}</p>
           </div>
           <button id="showMore" class="btn btn-primary m-2">Episodes</button>
         </div>
       </div>
      `);
...
$('#shows-list').on('click', '#showMore', async function(e){
  e.preventDefault()
...
function populateEpisodes(episondes){
  const epList = $('#episodes-list');
  epList.empty()

  for(let episode of episondes){
    
    let allEpisodes = $(`
      <li>
        ${episode.summary} (${episode.season} ${episode.number})
        
      </li>
    `)
    $('#episodes-area').css({display: 'block'})
    epList.css({'background-color': 'rgb(221, 221, 177)'})
    epList.append(allEpisodes)
  }
}